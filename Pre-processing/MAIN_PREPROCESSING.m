...
function MAIN_PREPROCESSING(args)
    ...
    if INITIAL_STEP >= 1
        if ~isNIfTI % the patients are saved as DICOM file
            ...
            if INITIAL_STEP == 1
                ...
                if exist(strcat(workspaceFolder, 'Image', suffix_workspace, '.mat'), 'file') == 2
                    ...
                    if SAVE_INTERMEDIATE_STEPS
                        suffix = "01_orig";
                        saveRegisteredImages(Image, saveRegisteredFolder, patients, suffix, previousNumPatiens, newIDFormat, directory, SAVE_AS_TIFF);
                    end
                end
            end
            ...
            if INITIAL_STEP == 2
                ...
                if exist('ImageRegistered','var')==0 && exist(strcat(workspaceFolder, 'ImageRegistered', suffix_workspace, '.mat'), 'file') == 2
                    ...
                    if SAVE_INTERMEDIATE_STEPS
                        ...
                        saveRegisteredImages(ImageRegistered, saveRegisteredFolder, patients, suffix, previousNumPatiens, newIDFormat, directory, SAVE_AS_TIFF);
                    end
                end
                toc
            end
    ...
    if INITIAL_STEP == 3
        ...
        if exist('ImageSkullRemoved','var')==0 && exist(strcat(workspaceFolder, 'ImageSkullRemoved', suffix_workspace, '.mat'), 'file') == 2
            ...
            ImageSkullRemoved = combinedTechniquesSkullRemoval(ImageRegistered, thold, patients, directory, SAVE, workspaceFolder, suffix_workspace, isNIfTI, ISLISTOFDIR, args.SAVE_AS_TIFF, saveRegisteredFolder);
    ...
    % %         ImageSkullRemoved = generalSkullRemoval(ImageRegistered, patients, SAVE, workspaceFolder);
            if SAVE_INTERMEDIATE_STEPS
                ...
                saveRegisteredImages(ImageSkullRemoved, saveRegisteredFolder, patients, suffix, previousNumPatiens, newIDFormat, directory, SAVE_AS_TIFF);
            end
        end
        toc
    end
    ...
    if INITIAL_STEP == 6
        ...
        saveRegisteredImages(NewImageRegistered, saveRegisteredFolder, patients, suffix, previousNumPatiens, newIDFormat, directory ,SAVE_AS_TIFF);
        toc 
    end
end
